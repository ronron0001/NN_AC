...
class Fig2Callback(keras.callbacks.Callback):
    def __init__(self, G_test, A_test, G_track, A_track, d_omega, checkpoint_epochs=None):
        super().__init__()
        self.G_test = G_test
        self.A_test = A_test  # 密度 (∫A dω = 1)
        self.G_track = G_track
        self.A_track = A_track  # 密度 (∫A dω = 1)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        pred_density = pred_prob / self.d_omega  # 密度 (∫A dω = 1)
        eps = 1e-10
        # 論文式(6): D_KL = Σ A_true ln(A_true/A_pred) = CE - H(A_true)
        # 密度形式: A_true, A_pred はともに密度
        kld_list = []
        for i in range(len(self.A_test)):
            ce = -np.sum(self.A_test[i] * self.d_omega * np.log(pred_density[i] + eps))
            h = -np.sum(self.A_test[i] * self.d_omega * np.log(self.A_test[i] + eps))
            kld_list.append(ce - h)  # KL = CE - H
        kld = np.mean(kld_list)
        mae = np.mean(np.abs(self.A_test - pred_density))
        mse = np.mean((self.A_test - pred_density) ** 2)
        rmse = np.sqrt(mse)
